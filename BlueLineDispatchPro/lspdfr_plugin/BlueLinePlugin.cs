...
{
    // Shared folder — must match lspdfr_bridge_path in ai_config.json
    private static readonly string SharedDir = Path.Combine(
        Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
        "BlueLineDispatch");

    private static string QueryFile    => Path.Combine(SharedDir, "plate_query.txt");
    private static string ResponseFile => Path.Combine(SharedDir, "plate_response.json");
    ...
    private void OnDutyChanged(bool onDuty)
    {
        if (onDuty)
        {
            ...
            Game.DisplayNotification(
                "~b~BlueLineDispatch~w~ plate bridge ~g~active");
        }
    }
    ...
    private void MainLoop()
    {
        ...
        while (true)
        {
            ...
            try
            {
                ...
                Game.LogTrivial($"[BlueLineDispatch] Plate query: {plate}");
    ...
    private string LookupPlate(string plate)
    {
        ...
        if (driver != null && driver.IsValid() && driver.Exists())
        {
            ...
            dob        = $"{p.BirthMonth:D2}/{p.BirthDay:D2}/{p.BirthYear}";
    ...
    {
        Game.LogTrivial("[BlueLineDispatch] Plugin unloaded.");
    }
}
